...
def generate_release_description(git_log: str, version: str, api_key: str) -> str:
    ...
    **NOTE**: Not all releases will include information for every section. Only include sections that are relevant based on the changes in this release.
    ...
    If no significant changes are found, state that clearly. Keep in mind, you dont have access to the frontend changes,
    so you may not have the full scope of the user-facing changes.
...
"""
    pre_disclaimer = "> **NOTE:** The above summary is generated by AI, it does not have context on frontend " + \
                     "changes, so it may not capture everything that changed."
    # Print the complete release notes with final length check
    final_output = pre_disclaimer + "\n\n" + description + installation_instructions
